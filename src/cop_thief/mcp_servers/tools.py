...
def send_message(from_agent: str, text: str, token: str) -> dict:
    """T-P2-01: Send a free-text message to the opponent."""
...
def receive_message(for_agent: str, token: str) -> dict:
    """T-P2-02: Retrieve the latest message for the agent."""
...
def update_position(agent: str, pos: list[int] | tuple[int, int], token: str) -> dict:
    """T-P2-03: Update the agent's position on the board."""
...
def verify_position(agent: str, token: str) -> dict:
    """T-P2-04: Retrieve the current position of the agent."""
    authorize(token, expected_agent=agent)
    if agent not in ("cop", "thief"):
        raise ValueError(f"Invalid agent '{agent}'")
    state = load_state()
...
def choose_action(agent: str, observation: dict, token: str) -> dict:
    """T-P2-05: Pick a legal action for the agent."""
    authorize(token, expected_agent=agent)
    if agent not in ("cop", "thief"):
        raise ValueError(f"Invalid agent '{agent}'")
    state = load_state()
...
def apply_action(agent: str, action: str, token: str) -> dict:
    """T-P2-06: Resolve and execute the agent's action."""
    authorize(token, expected_agent=agent)
    if agent not in ("cop", "thief"):
        raise ValueError(f"Invalid agent '{agent}'")
    try:
...
def game_status(token: str) -> dict:
    """T-P2-07: Get the current game status snapshot."""
    authorize(token)
    state = load_state()
