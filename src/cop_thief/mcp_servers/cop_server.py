"""Cop MCP Server exposing tools only.

Traces: FR-MCP1, FR-MCP5, FR-C1, T-P2-09, T-P2-10.
"""
...
@mcp.tool()
def place_barrier(token: str) -> dict:
    """Cop-only: Place a barrier on the cop's current cell."""
    return tools.apply_action(agent="cop", action="place_barrier", token=token)
